...
function keyField(collection: string): string {
  if (collection === "users" || collection === "projects" || collection === "ratings") {
    return "numericId";
  }
...
class QueryRef {
  ...
  where(field: string, op: FilterOp, value: unknown): QueryRef {
    return new QueryRef(this.collection, [...this.filters, { field, op, value }], this.orders, this.maxRows);
  }

  orderBy(field: string, direction: "asc" | "desc" = "asc"): QueryRef {
    return new QueryRef(this.collection, this.filters, [...this.orders, { field, direction }], this.maxRows);
  }
  ...
  get firestore(): { batch: () => { delete: (ref: DocRef) => void; commit: () => Promise<void> } } {
    return {
      batch: () => {
  ...
  async get(): Promise<QuerySnapshot> {
    ...
    const docs = (data ?? []).map((row) => {
      ...
      return new QueryDocumentSnapshot(new DocRef(this.collection, id), row as Row);
    });
